#!/bin/bash
# Launch Kinect v2 with RTABMap SLAM + IMU fusion
#
...
echo "=========================================="
echo "Kinect v2 + RTABMap SLAM + IMU Fusion"
...
cleanup() {
    echo ""
    echo "Shutting down..."
    kill $IMU_PID $KINECT_PID $IMU_FILTER_PID $RTABMAP_PID $RVIZ_PID 2>/dev/null
    wait $IMU_PID $KINECT_PID $IMU_FILTER_PID $RTABMAP_PID $RVIZ_PID 2>/dev/null
...
# Start IMU node (example for BNO055)
echo "1. Starting IMU node..."
ros2 run bno055 bno055_node &
IMU_PID=$!
sleep 2

# Start IMU filter for better orientation estimation
...
# Start RTABMap with ICP-based odometry + IMU fusion (precision mode)
echo "5. Starting RTABMap SLAM with ICP odometry + IMU (precision mode)..."
...
# Wait for processes
wait $IMU_PID $KINECT_PID $IMU_FILTER_PID $RTABMAP_PID $RVIZ_PID
